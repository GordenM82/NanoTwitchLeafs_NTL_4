...
def main():
    ...
    if '<Border Height="36"' not in main or 'consoleAutoScroll_CheckBox' not in main: errors.append("console auto-scroll alignment is missing")
    if 'TwitchLinkAvatar_Image" Height="105"' not in main: errors.append("Twitch layout was not compacted")
    if 'test_Column" Header="" Width="42"' not in trigger: errors.append("trigger action columns were not compacted")
    if info.count('TextAlignment="Center"') < 7: errors.append("information heading/developer block is not centered")
    if "4.1.0-layout-preview.26" not in read("Properties/AssemblyInfo.cs"): errors.append("assembly version is not Preview 26")
    if "layout-preview.26-win-x64" not in read(".github/workflows/build-release.yml"): errors.append("workflow artifact is not Preview 26")
    if errors:
