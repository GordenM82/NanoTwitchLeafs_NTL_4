...
namespace NanoTwitchLeafs.Controller
{
	public class TriggerLogicController 
	{
			...
			Task.Run(async () =>
			{
				...
				while (!token.IsCancellationRequested)
				{
					try
					{
						...
						{
							await SendColorToSinglePanel(queueObject.Color, queueObject.TriggerSetting.Brightness, queueObject.TriggerSetting);
						}
						else
						{
		...
		private async Task SendColorToSinglePanel(Color color, int brightness, TriggerSetting triggerSetting)
		{
			var random = new Random();
			var targetDevices = GetTargetDevices(triggerSetting);
			if (targetDevices.Count == 0)
			{
				_logger.Warn($"Trigger {triggerSetting?.ID} has no available Nanoleaf target devices.");
				return;
			}
			...
			foreach (var device in targetDevices)
			{
				try
				{
					var layout = device.NanoleafControllerInfo?.panelLayout?.layout;
					if (layout?.positionData == null || layout.numPanels <= 0 || layout.positionData.Count == 0)
					{
						_logger.Warn($"No usable panel layout for Device {device.PublicName} ({device.Address}).");
						continue;
					}

					var panelCount = Math.Min(layout.numPanels, layout.positionData.Count);
					var randomIndex = random.Next(0, panelCount);
					var panelId = layout.positionData[randomIndex].panelId;
				...
				{
					_logger.Error($"Could not set username color on Device {device.PublicName}. Other target devices will continue.", ex);
				}
			}
		}
		...
		{
			var targetDevices = GetTargetDevices(queueObject.TriggerSetting);
			if (targetDevices.Count == 0)
			{
				_logger.Warn($"Trigger {queueObject.TriggerSetting.ID} has no available Nanoleaf target devices.");
				return;
			}

			_logger.Debug($"Set Effect '{queueObject.TriggerSetting.Effect}' for {queueObject.TriggerSetting.Duration} Seconds on {targetDevices.Count} Devices");
			...
			foreach (var device in targetDevices)
			{
				try
				{
					...
					{
						_logger.Debug($"Save Current state '{currentState.Effect}' from Device {device.PublicName}");
						currentStates[device] = currentState;
					}
				}
				catch (Exception ex)
				{
					_logger.Error($"Could not read current state from Device {device.PublicName}. The trigger will still be attempted.", ex);
				}
			}
			...
			foreach (var device in targetDevices)
			{
				...
				{
					_logger.Error($"Could not apply trigger to Device {device.PublicName}. Other target devices will continue.", ex);
				}
			}

			if (successfulDevices > 0 && !queueObject.Username.Contains("TwitchEvent"))
			{
				string response;
				if (!queueObject.TriggerSetting.IsColor)
				{
			...
			if (successfulDevices > 0 && queueObject.TriggerSetting.Duration != 0 && currentStates.Count > 0)
			{
				await Task.Delay(Convert.ToInt32(queueObject.TriggerSetting.Duration) * 1000);

				_logger.Debug($"Change back to last state on {targetDevices.Count} Devices");
				foreach (var stateEntry in currentStates)
				{
					var device = stateEntry.Key;
					var oldState = stateEntry.Value;
					try
					{
						...
						if (oldState.Effect != "*Solid*")
						{
							...
							if (oldState.State?.brightness != null)
								await _nanoController.SetBrightness(device, oldState.State.brightness.value);
							if (oldState.State?.on != null && !oldState.State.on.value)
		...
		private List<NanoLeafDevice> GetTargetDevices(TriggerSetting triggerSetting)
		{
			var allDevices = _appSettings.NanoSettings.NanoLeafDevices;
			if (triggerSetting == null || string.IsNullOrWhiteSpace(triggerSetting.TargetDeviceNames))
			...
			var targets = allDevices.Where(device => selectedNames.Contains(device.DeviceName)).ToList();
			foreach (string missingDevice in selectedNames.Where(name => targets.All(device =>
				!string.Equals(device.DeviceName, name, StringComparison.OrdinalIgnoreCase))))
			{
				_logger.Warn($"Configured Nanoleaf target '{missingDevice}' is no longer paired and will be skipped.");
			}
			return targets;
		}
