...
public class TwitchEventSubController : IDisposable
{
    ...
    { 
        ...
        {
            _logger.Error("EventSub subscriptions skipped: Broadcaster user ID could not be resolved. Please relink the Twitch broadcaster account.");
            return;
        }
        var followConditions = new Dictionary<string, string>()
        {
            { "broadcaster_user_id", broadcasterId },
            { "moderator_user_id", broadcasterId }
        };
        var broadcasterConditions = new Dictionary<string, string>()
        {
            { "broadcaster_user_id", broadcasterId }
        };
        ...
        var subscriptionResults = new List<bool>
        {
            await CreateSubscriptionAsync("channel.follow", "2", followConditions),
            await CreateSubscriptionAsync("channel.subscribe", "1", broadcasterConditions),
            await CreateSubscriptionAsync("channel.subscription.message", "1", broadcasterConditions),
            await CreateSubscriptionAsync("channel.subscription.gift", "1", broadcasterConditions),
            await CreateSubscriptionAsync("channel.cheer", "1", broadcasterConditions),
            await CreateSubscriptionAsync("channel.raid", "1", raidCondition),
            await CreateSubscriptionAsync("channel.channel_points_custom_reward_redemption.add", "1", broadcasterConditions),
            await CreateSubscriptionAsync("channel.channel_points_custom_reward_redemption.update", "1", broadcasterConditions),
            await CreateSubscriptionAsync("channel.hype_train.begin", "2", broadcasterConditions),
            await CreateSubscriptionAsync("channel.hype_train.progress", "2", broadcasterConditions),
            await CreateSubscriptionAsync("channel.hype_train.end", "2", broadcasterConditions)
        };

        var successfulSubscriptions = subscriptionResults.FindAll(result => result).Count;
        _logger.Info($"EventSub subscriptions active: {successfulSubscriptions}/{subscriptionResults.Count}");
    }

    private async Task<bool> CreateSubscriptionAsync(string type, string version, Dictionary<string, string> conditions)
    {
        try
        {
            await _api.Helix.EventSub.CreateEventSubSubscriptionAsync(type, version, conditions,
                EventSubTransportMethod.Websocket, _eventSubWebsocketClient.SessionId);
            _logger.Info($"EventSub subscribed: {type} v{version}");
            return true;
        }
        catch (HttpResponseException ex)
        {
            var response = await ex.HttpResponse.Content.ReadAsStringAsync();
            _logger.Error($"EventSub subscription failed: {type} v{version} - {response}");
            return false;
        }
        catch (Exception ex)
        {
            _logger.Error($"EventSub subscription failed: {type} v{version} - {ex.Message}", ex);
            return false;
        }
    }
